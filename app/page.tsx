...
export default function Home() {
  ...
  const importExcel = async (file: File) => {
    try {
      ...
      const dataRows = grid.slice(1).filter(row=>row.some(cell=>cell!==""));
      if (!dataRows.length) throw new Error("Sheet kosong");
      const headers = headerRow.map((x) => x.toLowerCase()).join("|");
      const take=(row:unknown[],names:string[])=>{const index=headerRow.findIndex(key=>names.some(name=>key.toLowerCase()===name.toLowerCase()));return index>=0?row[index]:""};
      ...
      if (headers.includes("video id") && headers.includes("campaign name")) {
        ...
        if(importKind!=="Livestream") throw new Error("File ini terbaca sebagai Livestream. Ubah jenis data menjadi Livestream.");
        const mapped = dataRows.map((r) => { const launchedAt=String(take(r,["Launched time"])); const cost=n(take(r,["Cost"])); const revenue=n(take(r,["Gross revenue (Current Shop)","Gross revenue","Revenue"])); return { brand:targetBrand, period:importPeriod, name:String(take(r,["LIVE name"])), launchedAt, day:launchedAt.slice(0,10), campaign:String(take(r,["Campaign name"])), campaignId:String(take(r,["Campaign ID"])), status:String(take(r,["Status"])), cost, revenue, orders:n(take(r,["SKU orders (Current Shop)","SKU orders","Orders"])), views:n(take(r,["LIVE views","Views"])), tenSecondViews:n(take(r,["10s views"])), follows:n(take(r,["LIVE follows","Follows"])), durationMinutes:n(take(r,["LIVE duration (min)","Duration (min)","Duration minutes","LIVE duration"])), roi:cost?revenue/cost:0, importId } as LiveRow; });
  ...
  return <main className="app-shell">
    ...
    <header className="topbar">
      ...
      <div className="top-actions"><button className="report-button" onClick={()=>window.print()}>Export ringkasan</button><button className="import-button" onClick={()=>setImportOpen(true)}>Import Excel</button><div className="period"><span>REPORTING PERIOD</span><b>01—28 FEB 2026</b></div></div>
    </header>
