...
export default function Home() {
  ...
  const importExcel = async (file: File) => {
    try {
      ...
      const grid = XLSX.utils.sheet_to_json<unknown[]>(workbook.Sheets[workbook.SheetNames[0]], { header: 1, defval: "", raw: true });
      const normalizeHeader=(value:unknown)=>String(value??"").replace(/^\uFEFF/,"").replace(/[_\s]+/g," ").trim().toLowerCase();
      const headerIndex=grid.slice(0,25).findIndex(row=>{const keys=row.map(normalizeHeader);const creative=(keys.includes("video id")||keys.includes("post id"))&&(keys.includes("campaign name")||keys.includes("creative")||keys.includes("video title"));const live=keys.some(key=>["live name","livestream name"].includes(key))&&keys.some(key=>["launched time","launch time","live start time"].includes(key));return creative||live});
      ...
      if (has("Video ID","Post ID") && has("Campaign name","Creative","Video title")) {
        setImportKind("Creative");
        const mapped = dataRows.map((r) => { const cost = n(take(r,["Cost"])); const revenue = n(take(r,["Gross revenue","Revenue"])); return {
          brand:targetBrand, period:importPeriod, campaign:String(take(r,["Campaign name"])||file.name.match(/Campaign\s+([^.]*)/i)?.[1]||"Creative import"), campaignId:String(take(r,["Campaign ID"])), productId:String(take(r,["Product ID"])), type:String(take(r,["Creative type","Type"])||"Video"), title:String(take(r,["Video title","Video caption","Creative","Creative name","Title"])), videoId:String(take(r,["Video ID","Post ID"])), account:String(take(r,["TikTok account","Account","Creator username"])), postedAt:String(take(r,["Time posted","Video post time","Posted time"])), status:String(take(r,["Status"])), authorization:String(take(r,["Authorization type","Authorization status","Authorization"])), cost, revenue, orders:n(take(r,["SKU orders","Orders"])), impressions:n(take(r,["Product ad impressions","Impressions"])), clicks:n(take(r,["Product ad clicks","Clicks"])), ctr:n(take(r,["Product ad click rate","CTR"])), cvr:n(take(r,["Ad conversion rate","CVR"])), view2:n(take(r,["2-second ad video view rate","2-second video view rate"])), view6:n(take(r,["6-second ad video view rate","6-second video view rate"])), view25:n(take(r,["25% ad video view rate","Video views at 25%"])), view50:n(take(r,["50% ad video view rate","Video views at 50%"])), view75:n(take(r,["75% ad video view rate","Video views at 75%"])), view100:n(take(r,["100% ad video view rate","Video views at 100%"])), roi:cost?revenue/cost:0, importId
        } as CreativeRow; });
        const replaced=importHistory.filter(x=>x.brand===targetBrand&&x.kind==="Creative"&&x.period===importPeriod);
        setCreativeSource(prev=>[...prev.filter(x=>!replaced.some(record=>record.id===x.importId)&&!(replaced.some(record=>record.builtin)&&!x.brand)),...mapped]); setImportHistory(prev=>[{id:importId,brand:targetBrand,file:file.name,kind:"Creative",period:importPeriod,rows:dataRows.length,importedAt:new Date().toLocaleString("id-ID")},...prev.filter(x=>!(x.brand===targetBrand&&x.kind==="Creative"&&x.period===importPeriod))]); setImportMessage(`${dataRows.length.toLocaleString("id-ID")} baris creative ${monthLabel(importPeriod)} ${replaced.length?"menggantikan data lama":"ditambahkan"} ke ${targetBrand}.`); setImportOpen(false); setTab("overview");
      } else if (has("LIVE name","Livestream name") && has("Launched time","Launch time","LIVE start time")) {
        setImportKind("Livestream");
        const mapped = dataRows.map((r) => { const launchedAt=String(take(r,["Launched time","Launch time","LIVE start time"])); const cost=n(take(r,["Cost","Spend"])); const revenue=n(take(r,["Gross revenue (Current Shop)","Gross revenue","Revenue","GMV"])); return { brand:targetBrand, period:importPeriod, name:String(take(r,["LIVE name","Livestream name"])), launchedAt, day:launchedAt.slice(0,10), campaign:String(take(r,["Campaign name","LIVE campaign name"])), campaignId:String(take(r,["Campaign ID","LIVE campaign ID"])), status:String(take(r,["Status","Delivery status"])), cost, revenue, orders:n(take(r,["SKU orders (Current Shop)","SKU orders","Orders"])), views:n(take(r,["LIVE views","Views","Livestream views"])), tenSecondViews:n(take(r,["10s views","10-second views"])), follows:n(take(r,["LIVE follows","Follows"])), durationMinutes:n(take(r,["LIVE duration (min)","Duration (min)","Duration minutes","LIVE duration"])), roi:cost?revenue/cost:0, importId } as LiveRow; });
        ...
        const replaced=importHistory.filter(x=>x.brand===targetBrand&&x.kind==="Livestream"&&x.period===importPeriod);
        setLiveSource(prev=>[...prev.filter(x=>!replaced.some(record=>record.id===x.importId)&&!(replaced.some(record=>record.builtin)&&!x.brand)),...mapped]); setImportHistory(prev=>[{id:importId,brand:targetBrand,file:file.name,kind:"Livestream",period:importPeriod,rows:dataRows.length,importedAt:new Date().toLocaleString("id-ID")},...prev.filter(x=>!(x.brand===targetBrand&&x.kind==="Livestream"&&x.period===importPeriod))]); setImportMessage(`${dataRows.length.toLocaleString("id-ID")} sesi livestream ${monthLabel(importPeriod)} ${replaced.length?"menggantikan data lama":"ditambahkan"} ke ${targetBrand}.`); setImportOpen(false); setTab("overview");
