...
export default function Home() {
  ...
  const creativeRows = useMemo(() => brandCreative
    ...
    .sort((a, b) => b[sort] - a[sort]), [brandCreative, campaign, query, onlySpend, sort]);
  ...
  const overviewLive = brandLive.reduce((a,x)=>({cost:a.cost+x.cost,revenue:a.revenue+x.revenue,orders:a.orders+x.orders,rowCount:a.rowCount+1,views:a.views+x.views}),{cost:0,revenue:0,orders:0,rowCount:0,views:0});
  ...
  return <main className="app-shell">
    ...
    {tab === "overview" && <>
      <section className="overview-filter"><div><b>Filter overview</b><span>Brand aktif: {selectedBrand==="all"?"Semua brand":selectedBrand}</span></div><label>Data<select value={overviewScope} onChange={(e)=>setOverviewScope(e.target.value as typeof overviewScope)}><option value="all">Creative + Live</option><option value="creative">Creative saja</option><option value="live">Live saja</option></select></label><label>Campaign creative<select value={overviewCampaign} onChange={(e)=>setOverviewCampaign(e.target.value)}><option value="all">Semua campaign</option>{creativeCampaigns.map(x=><option key={x.name}>{x.name}</option>)}</select></label><button onClick={()=>{setOverviewScope("all");setOverviewCampaign("all")}}>Reset</button></section>
    ...
    {tab === "creative" && <>
      ...
      <section className="panel table-panel">
        <div className="panel-head"><div><span>VIDEO ID OVERVIEW</span><h2>{number(creativeRows.length)} video sesuai filter</h2></div><div className="roi-legend"><span><i className="good"/>ROI ≥ 6 sangat bagus</span><span><i className="mid"/>ROI 2–5,99 perlu cek</span><span><i className="bad"/>ROI ≤ 2 buruk</span></div></div>
    ...
    {tab === "live" && <>
      ...
      <section className="kpi-grid six">
        ...
        <Kpi label="Avg durasi live" value={liveAgg.durationRows?`${number(liveAgg.durationMinutes/liveAgg.durationRows)} mnt`:"—"} note={liveAgg.durationRows?`Total ${number(liveAgg.durationMinutes/60)} jam`:"Kolom durasi tidak ada di export"} />
      </section>
